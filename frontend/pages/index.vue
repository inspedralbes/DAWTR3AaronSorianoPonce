<template>
  <div class="min-h-screen bg-gray-950 text-white font-sans selection:bg-purple-500/30">
    <!-- Component de navegació global -->
    <CapcaleraWeb transparent />
    
    <main class="max-w-7xl mx-auto px-4 py-12">
      <!-- Secció Hero: Atraure l'usuari amb el missatge principal. -->
      ...
      <!-- Cartellera d'Esdeveniments: Llistat interactiu de butaques disponibles. -->
      ...
      <!-- Indicador de càrrega per a una millor UX. -->
      <div v-if="pending" class="flex gap-4 p-8 justify-center">
         <div class="w-8 h-8 border-4 border-purple-500 border-t-transparent rounded-full animate-spin"></div>
      </div>
      
      <!-- Graella d'espectacles disponibles obtinguts des del backend. -->
...
onMounted(() => {
    // Inicialització de les traduccions al primer llançament.
    i18n.init()
})

/**
 * Crida reactiva a l'API de Laravel per obtenir el llistat d'esdeveniments.
 * Ús de composables de Nuxt 3 per a la gestió d'estats de càrrega.
 */
const { data: events, pending } = useFetch(`${config.public.socketUrl}/api/events`)

/**
 * Navegació cap al detall d'un esdeveniment específic.
 */
const goToEvent = (id) => {
